...
| SCALE-02 | P0 | planned | Replace 24-at-a-time roll hydration that rebuilds and retransmits the full archive after every batch. | Roll analysis is a bounded background job with lightweight progress; a 20k archive never performs O(n²) snapshot work and remains responsive. |
| SCALE-03 | P0 | planned | Page or virtualize copy-heavy views and query them without mounting the entire archive. | Transfers, retrieval history, ingestion history, quarantine, and any copy list keep a bounded DOM and meet the performance budgets below with 20k+ copies. |
...
| NOTICE-01 | P1 | planned | Replace independent snackbar refs/timers with one notification service. | Notifications are queued or coalesced, never overlap, use stable severity/action semantics, remain accessible, and preserve important transfer/recovery messages until dismissed. |
