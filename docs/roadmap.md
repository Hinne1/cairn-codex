...
| ONBOARD-01 | P0 | done | Add guided first-run onboarding. | A new user can locate Grim Dawn, choose “Import Item Assistant” or “Continue without importing,” understand archive versus live transfer, and reach Collection without reading the README. The no-import path explicitly preserves the existing archive, stashes, saves, and Item Assistant data; the tour can be skipped and reopened. |
...
| TRANSFER-01 | P1 | done | Redesign Transfers around audit and exception recovery rather than archive browsing. | Tabs are **Ingest history**, **Dispense history**, and **Quarantined items**. Both histories are read-only journal views with search/paging plus SC/HC, seed, time, outcome, and correlation ID; only quarantined copies expose return actions. |
| SEARCH-01 | P1 | done | Build one shared query parser and search-document contract. | Every ExplorerToolbar supports implicit AND, explicit `AND`/`OR`, quoted phrases, negation, and documented fields such as `skill:`, `damage:`, `slot:`, `rarity:`, `level:`, `owned:`, and `affix:` where meaningful. |
| SEARCH-02 | P1 | done | Add an inline search guide and examples. | Help is reachable from every search bar, explains scope for the current tool, and can insert example queries such as `skill:wendigo AND "vitality damage"`. |
...
| PLANNER-01 | P1 | done | Redesign Leveling Planner creation. | “New plan” offers Blank, Character save, and Clone; users choose masteries/build identity, skills, and level range in a short guided flow, then edit through a compact consistent toolbar. |
...
| ARCH-04 | P1 | done | Introduce a background-job contract. | Scan, roll hydration, import, backup, icon extraction, and map indexing report typed progress, support cancellation where safe, avoid duplicate jobs, and survive navigation. |
