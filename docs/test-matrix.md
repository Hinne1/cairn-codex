...
| Collection Farming bounded routes | `npm run test:farming`, exact 214-route source-build fixture, 1,440×1,000 and 520×900 captures, normal/empty searches, and automated paging/filter/tooltip interaction | **Passed 2026-09-01:** the ranked fixture reported 214 useful areas while mounting exactly 50 route cards at both widths; the final model page retained global ranks 201–214. Next received keyboard focus and replaced page one with ranks 51–100; changing rarity reset the view to rank 1. A one-route search retained 12 item snippets plus its `+1 more` summary and a zero-route search used the shared empty state. Item snippets retained the global tooltip, both widths had zero document-level horizontal overflow, and route keys remained stable across pages. |
