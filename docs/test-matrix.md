...
| Shared bounded result contract | `npm run test:bounded-results`, renderer typecheck/build, synthetic 20k/50k arrays | **Passed 2026-09-01 incremental slices:** local 20,000- and 50,000-record collections mounted exactly one 50-record page; an over-delivered server page was capped at 50; page clamping, stable-key rejection, linear/grid keyboard movement, and none/single/multi keyed selection passed. Transfers history and quarantine use the shared loading/empty/range/paging surface; quarantine additionally preserves disabled selection while a recovery is busy. Empty quarantine remained readable at 1,440×1,000 and 520×900. Remaining result-heavy tools stay tracked by #21. |
