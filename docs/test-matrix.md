...
| Main-process service boundaries | `npm run test:ipc-services` and `npm run test:domain-services` with fake IPC, storage, helper, diagnostics, app, and window adapters | **Passed 2026-09-01:** 51 request channels have unique ownership across 8 concrete services; invalid inputs make zero service calls; valid inputs delegate once; ordinary known and unexpected failures map to redacted, serializable domain errors; rejected writes do not poison queues; import cancellation is pre-mutation and committed imports schedule protection; collection hydration stays in 256-item batches; archive mutations serialize; transfer timeout, restart, replay, rejection, and uncertain outcomes fail closed; repeated quit events run one flush workflow; preload channels remain unchanged |
