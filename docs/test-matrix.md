...
| Shared bounded result contract | `npm run test:bounded-results`, renderer typecheck/build, synthetic 20k/50k arrays, isolated MI Workshop and Skill Explorer fixtures | **Passed 2026-09-01 incremental slices:** local 20,000- and 50,000-record collections mounted exactly one 50-record page; an over-delivered server page was capped at 50; page clamping, stable-key rejection, linear/grid keyboard movement, and none/single/multi keyed selection passed. Transfers history and quarantine use the shared loading/empty/range/paging surface; quarantine additionally preserves disabled selection while a recovery is busy. MI Workshop reported all 72 synthetic affix combinations while mounting exactly 50 rows at 1,440×1,000 and 520×900. Skill Explorer reported 120 synthetic Wendigo Totem items while mounting exactly 50 rows at both widths; its empty search state remained readable. Focus opened the shared item tooltip, ArrowDown/End/Home moved focus, and Next/Previous replaced then restored the bounded page. The remaining Planner migration stays tracked by #21. |
