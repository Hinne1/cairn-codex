...
| Main-process service boundaries | `npm run test:ipc-services` with fake IPC, service, diagnostics, app, and window adapters | **Passed 2026-09-01:** 51 request channels have unique ownership across 8 domains; invalid inputs make zero service calls; valid inputs delegate once; known and unexpected failures cross as typed service errors; serialized writes continue after rejection; unresolved transfer recovery fails closed; repeated quit events run one flush workflow; preload channels remain unchanged |
