...
import { buildMiMetricOptions } from '../../src/renderer/src/workspaces/mi-workshop'
  ...
  return () => h('main', { style: 'padding:16px;min-width:0' }, [
    workspace.value === 'collection' ? [
      ...
      h(CollectionMaterials, { mode: 'collection', items: snapshot.value?.items ?? [], controls: controls.value,
        'onUpdate:controls': value => { controls.value = value }, doubleRareMiBaseRecords: new Set(),
    ...
    h(Drawer, { session: inspection, metric: metric.value, 'onUpdate:metric': value => { metric.value = value },
