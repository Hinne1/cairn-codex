...
assert.match(workspace, /value\.startsWith\('roll-'\)[\s\S]*?ownership: ownership\.value === 'favorite' \? 'favorite' as const : 'owned' as const/)
assert.match(workspace, /watch\(\[\(\) => props\.mode, sort\][\s\S]*?ownership\.value = 'owned'[\s\S]*?immediate: true/)
assert.match(workspace, /:scores="rollCategoryScores\(rollSummary\(item\)!\.copy\.rollAnalysis\)"/)
