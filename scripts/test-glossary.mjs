...
const miExplanation = entry.sections.find(section => section.id === 'choosing-copies').paragraphs.find(paragraph => paragraph.startsWith('MI Workshop'))
assert.match(miExplanation, /highest-scoring copy for the selected metric always leads its group/)
assert.match(miExplanation, /Sort direction changes the order of groups, not which copy leads each group/)
