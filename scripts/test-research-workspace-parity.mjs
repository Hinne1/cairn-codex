...
const plannerWorkspace = await readFile(new URL('../src/renderer/src/workspaces/LevelingPlannerWorkspace.vue', import.meta.url), 'utf8')
const plannerSession = await readFile(new URL('../src/renderer/src/workspaces/leveling-planner.ts', import.meta.url), 'utf8')
const plannerResults = await readFile(new URL('../src/renderer/src/workspaces/planner-results.ts', import.meta.url), 'utf8')
...
assert.match(plannerResults, /available: researchItemIsAvailable\(row.item, archivedRecords, recipe \? recipe.known === true/)
assert.match(plannerWorkspace, /<ResearchItemTable[\s\S]*?<PlannerJourney/)
assert.match(plannerWorkspace, /@sort="sortPlannerTable"/)
assert.match(plannerSession, /get: \(\) => plannerProfiles.value.find\([\s\S]*?\?\.ignoredRecords \?\? \[\]/)
assert.doesNotMatch(plannerSession, /initialPreferences.planner.ignoredRecords/)
assert.match(app, /tooltip-icon-placeholder/)
const inspectionDrawer = await readFile(new URL('../src/renderer/src/inspection/ItemInspectionDrawer.vue', import.meta.url), 'utf8')
assert.match(inspectionDrawer, /comparison-icon-placeholder[\s\S]*?copy-icon-placeholder/)
assert.match(plannerWorkspace, />Table<\/button>[\s\S]*?>Journey<\/button>[\s\S]*?>MI sources<\/button>/)
...
assert.match(table, /class="research-item"[\s\S]*?@mouseenter="!menuRequest && emit\('queue-tooltip', row\.item, \$event\)"[\s\S]*?@mouseleave="emit\('hide-tooltip'\)"[\s\S]*?@wheel="emit\('scroll-tooltip', \$event\)"/)
...
assert.match(journey, /class="planner-journey-picture"[\s\S]*?@mouseenter="!menuRequest && emit\('queue-tooltip', row\.item, \$event\)"/)
