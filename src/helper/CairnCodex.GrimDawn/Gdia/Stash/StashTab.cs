...
namespace IAGrim.Parser.Stash {
    public class StashTab {
        ...
        public bool Read(GDCryptoDataBuffer pCrypto, uint containerVersion) {
            ...
            bool result;
            if (failed) {
                LastError = $"Tab header failed at offset {pCrypto.Cursor}.";
                return false;
            }
            else {
                this.Items = new List<Item>();
                for (uint i = 0u; i < numItems; i += 1u) {
                    ...
                    if (flag2) {
                        LastError = $"Item {i} failed at offset {pCrypto.Cursor}.";
                        return false;
                    }
                    this.Items.Add(item);
                }
                ...
                if (flag3) {
                    LastError = $"Tab block ended at offset {pCrypto.Cursor}, expected {this._block.End}.";
                }
                result = !flag3;
            }
            return result;
        ...
        public void Write(DataBuffer pBuffer) {
            ...
            if (this.Version >= 9) {
                ...
                pBuffer.WriteWString(buttonName ?? string.Empty);
            }
